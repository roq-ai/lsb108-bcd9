...
import { useRoqClient, useInteractionFindFirst } from 'lib/roq';
...
function InteractionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useInteractionFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: InteractionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.interaction.update({
        data: values as RoqTypes.interaction,
        where: {
          id,
        },
      });
