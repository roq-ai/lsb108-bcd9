...
import { useRoqClient, useRoleFindFirst } from 'lib/roq';
...
function RoleEditPage() {
  ...
  const { data, error, isLoading, mutate } = useRoleFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: RoleInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.role.update({
        data: values as RoqTypes.role,
        where: {
          id,
        },
      });
