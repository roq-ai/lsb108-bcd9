...
import { useRoqClient, useRoleFindManyWithCount } from 'lib/roq';
...
export function RoleListPage(props: RoleListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [, 'user_role.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['role_name.contains', 'description.contains'],
          ...params.filters,
        },
        'role',
      ),
  ...
  const { data, error, isLoading, mutate } = useRoleFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.role.delete({ where: { id } });
      await mutate();
    } catch (error) {
