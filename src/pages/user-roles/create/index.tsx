...
function UserRoleCreatePage() {
  ...
  const formik = useFormik<UserRoleInterface>({
    initialValues: {
      assigned_at: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="assigned_at" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Assigned At
            </FormLabel>
            <DatePicker
              selected={formik.values?.assigned_at ? new Date(formik.values?.assigned_at) : null}
              onChange={(value: Date) => formik.setFieldValue('assigned_at', value)}
            />
          </FormControl>
          ...
          <AsyncSelect<RoleInterface>
            ...
            fetcher={() => roqClient.role.findManyWithCount({})}
