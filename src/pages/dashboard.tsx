...
import { useRoqPlatformUserProfiles, useInteractionCount, useRoleCount, useBotCount } from 'lib/roq';

import { RoleListPage as ShowCaseList } from 'pages/roles';

function DashboardPage() {
  const { session } = useSession();

  const interactionStat = useInteractionCount();

  const roleStat = useRoleCount();

  const botStat = useBotCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Interactions',
      result: interactionStat?.data,
      isLoading: interactionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Roles',
      result: roleStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`roles`} passHref legacyBehavior>
                  <Button
