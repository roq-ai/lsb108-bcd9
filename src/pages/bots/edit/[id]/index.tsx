...
import { useRoqClient, useBotFindFirst } from 'lib/roq';
...
function BotEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBotFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BotInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.bot.update({
        data: values as RoqTypes.bot,
        where: {
          id,
        },
      });
